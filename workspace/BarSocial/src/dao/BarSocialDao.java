...
public class BarSocialDao {
	...
	public static User searchUser(User user) throws SQLException, InstantiationException, IllegalAccessException, ClassNotFoundException {
		...
		if(lista.next()){
			...
			if (affectedRows == 0) {
				throw new SQLException("Erro na criação do usuário.");
		    }
			try (ResultSet generatedKeys = inserir.getGeneratedKeys()) {
				if (generatedKeys.next()) {
		        	...
		        	throw new SQLException("Não foi possivel obter o id do usuário.");
		        }
			}
			userToReturn = user;
		}
	...
	/*Checa o status de um pedido de um usuario especifico*/
	...
	/*Retorna o Primeiro Pedido que ainda não esta pronto */
